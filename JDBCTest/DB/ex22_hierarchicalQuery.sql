...
- 부모와 자식으로 구성된 트리 구조의 데이터 처리하는 질의
...
select c1.name as 부모부품, c2.name as 자식부품 from tblComputer c1
    inner join tblComputer c2
        --on c1.pseq = c2.seq; -- c1(자식), c2(부모)
        on c1.seq = c2.pseq; -- c1(부모), c2(자식)


-- 2. 계층형 쿼리
-- : strat with 절 connect by 절
select * from tblComputer
    start with seq = 1 --루트 노드 지정
        connect by prior seq = pseq; --부모 자식의 연결 고리
...
select lpad(' ', (level-1) * 5) || name, level, prior name, prior qty from tblComputer
    start with seq = 1
        connect by prior seq = pseq;
